export default {
  ...
  aiInsights: {
    ...
    on: 'Вкл',
    hintOn:
      'При добавлении записей и пересборке используется ИИ для симптомов и гипотез.',
    hintOff: 'Включите, чтобы отправлять текст журнала в настроенную LLM (через dev-прокси).',
    banner:
      'ИИ включён — новые записи анализируются; пересборка обновляет гипотезы по журналу.',
    unavailable:
  ...
  notifications: {
    ...
    regeneration: {
      ...
      slowNotificationMessage:
        'Гипотезы пересобираются из вашего журнала.',
      successTitle: 'Гипотезы готовы',
      successToastMessage:
        'Гипотезы готовы. Откройте раздел «Гипотезы».',
  ...
  hypothesesPage: {
    ...
    title: 'Гипотезы',
    subtitle:
      'Пересоздание обновляет гипотезы по областям на основе текущего журнала.',
    generate: 'Создать гипотезы',
    generateTitle: 'Построить гипотезы по журналу',
    regenerate: 'Пересоздать гипотезы',
    regenerating: 'Создание…',
    loading: 'Загрузка гипотез…',
    notGeneratedTitle: 'Пока ничего не создано',
    notGeneratedText:
      'Гипотезы создаются по журналу только после нажатия «Создать». При открытии страницы ничего не запускается автоматически.',
    noHypothesesTitle: 'Гипотез пока нет',
    noHypothesesText:
      'Добавьте записи в журнал, затем «Пересоздать» в шапке.',
    regenerateConfirmTitle: 'Пересоздать гипотезы?',
    regenerateConfirmMessage:
      'Все гипотезы будут заменены новыми на основе текущего журнала.',
    regenerateConfirmAction: 'Пересоздать',
    regenerateSlowHint:
      'Пересборка ещё идёт. Можно уйти со страницы — мы уведомим, когда гипотезы будут готовы.',
    ...
    regenerateMessages: {
      ...
      needEntries:
        'Сначала добавьте записи в журнал, затем пересоздайте гипотезы.',
      success:
        'Пересоздано гипотез: {hypothesisCount} из {journalEntryCount} записей журнала по {areaCount} зонам.',
      noHypotheses:
    ...
    assistant: {
      ...
      hasHistory: 'Сохранённый диалог',
      title: 'ИИ-помощник',
      subtitle:
        'Вопросы о журнале и сгенерированных гипотезах. Ответы через настроенную LLM.',
      emptyHint:
        'Спросите о закономерностях, что записать дальше или как подготовиться к визиту.',
      starters: [
        'Кратко объясни мои основные гипотезы',
        'О чём поговорить с врачом?',
        'Какой области тела не хватает записей в журнале?',
      ],
      you: 'Вы',
      assistant: 'Помощник',
      thinking: 'Думаю…',
      placeholder: 'Спросите о журнале или гипотезах…',
      inputLabel: 'Сообщение помощнику',
      send: 'Отправить',
      clear: 'Очистить чат',
      aiOff: 'Включите ИИ-анализ в навигации, чтобы пользоваться помощником.',
      needJournal: 'Добавьте хотя бы одну запись в журнал перед чатом.',
      disclaimer:
        'Ответы помощника носят информационный характер и не являются медицинским советом или диагнозом.',
    },
  },
  journalPage: {
    eyebrow: 'Журнал',
