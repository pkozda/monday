#!/usr/bin/env node
/**
 * Clears all journal entries, linked timeline events, and hypotheses (browser IndexedDB).
 *
 * Requires the dev server so the app can run on the same origin as MondayDB.
 *
 * Usage:
 *   npm run clear-journal
 *   npm run clear-journal -- --port 5174
 *
 * Manual (browser console while the app is open):
 *   await clearMondayJournal()
 */
...
async function main() {
  ...
  if (!server) {
    ...
    console.error('  npm run clear-journal')
    console.error('')
    console.error('If Vite uses another port, pass it explicitly:')
    console.error('  npm run clear-journal -- --port 5174')
    console.error('')
    console.error('Or clear manually in the browser console (app must be open):')
    console.error('  await clearMondayJournal()')
    process.exit(1)
  }

  const clearUrl = `http://${server.host}:${server.port}/?clearJournal=1&confirm=1`

  console.log('Opening the app to clear all journal records…')
  ...
  console.log(
    'Removes journal entries, their linked timeline events, and all hypotheses. Other timeline rows are kept.'
  )
}
